...
try {
  ...
  const content={...previous.content,products:previous.content.products.map(p=>{
    if(!['ciano-10','ciano-20','ciano-100'].includes(p.id) || p.colors==='') return p;
    changed++; return {...p,colors:''};
  })};
  ...
  if(changed){
    ...
    const result=await collection.updateOne({_id:'main',revision}, {
      $set:{content,updatedBy:'remove-ciano-color',updatedAt:new Date()},
      $inc:{revision:1},
  ...
  console.log(`${changed} colores de cianoacrilato eliminados. Otros datos conservados.`);
} catch {
  console.error('No se pudo completar la actualización del cianoacrilato. Revisá conexión, validación o ediciones simultáneas.');
  process.exitCode=1;
