...
try {
  ...
  const content={...previous.content,products:previous.content.products.map(p=>{
    const use=defaultContent.products.find(d=>d.id===p.id)?.use;
    if(!use || use===p.use) return p;
    changed++; return {...p,use};
  })};
  ...
  if(changed){
    ...
    const result=await collection.updateOne({_id:'main',revision}, {
      $set:{content,updatedBy:'requested-product-descriptions',updatedAt:new Date()},
      $inc:{revision:1},
  ...
  console.log(`${changed} descripciones de productos actualizadas. Otros datos conservados.`);
} catch {
  console.error('No se pudo completar la actualización de descripciones. Revisá conexión, validación o ediciones simultáneas.');
  process.exitCode=1;
