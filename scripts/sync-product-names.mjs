...
try {
  ...
  const content={...previous.content,products:previous.content.products.map(p=>{
    const name=defaultContent.products.find(d=>d.id===p.id)?.name;
    if(!name || name===p.name) return p;
    changed++; return {...p,name};
  })};
  ...
  if(changed){
    ...
    const result=await collection.updateOne({_id:'main',revision}, {
      $set:{content,updatedBy:'official-product-names',updatedAt:new Date()},
      $inc:{revision:1},
  ...
  console.log(`${changed} nombres de productos actualizados. Otros datos conservados.`);
} catch {
  console.error('No se pudo completar la actualización de nombres. Revisá conexión, validación o ediciones simultáneas.');
  process.exitCode=1;
