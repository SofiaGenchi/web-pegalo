...
try {
  ...
  const content={...previous.content,products:previous.content.products.map(p=>{
    const updated=defaultContent.products.find(d=>d.id===p.id);
    if(!updated || (updated.size===p.size && updated.colors===p.colors)) return p;
    changed++; return {...p,size:updated.size,colors:updated.colors};
  })};
  ...
  if(changed){
    ...
    const result=await collection.updateOne({_id:'main',revision}, {
      $set:{content,updatedBy:'requested-product-presentations',updatedAt:new Date()},
      $inc:{revision:1},
  ...
  console.log(`${changed} presentaciones de productos actualizadas. Otros datos conservados.`);
} catch {
  console.error('No se pudo completar la actualización de presentaciones. Revisá conexión, validación o ediciones simultáneas.');
  process.exitCode=1;
